...
Optionally, you can create `./config/config.js` file to override configuration default properties with your custom settings:
  * config.sources: your working directory where all projects will be cloned to. Default to `./src/node_modules`
  * config.log: the full path to a log file where to save logs. Default to `./logs/output.log`
2. Run command `npm install` inside cloned folder
...
* `npm install` again to autoupdate cta repo, clone new CTA repositories, update existing, install npm dependencies
